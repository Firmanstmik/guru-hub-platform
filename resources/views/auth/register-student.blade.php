@extends('layout.auth')
@section('title', 'Daftar Siswa — Guru Hub')
@section('auth-simple', true)

@section('brand-headline', 'Mulai perjalanan belajar Anda.')
@section('brand-subline', 'Kurikulum terstruktur dari pengajar terverifikasi — siap diakses setelah daftar.')

@section('auth-header')
    <x-auth.role-switcher active="student" />
    <h2 class="gh-auth-card-title gh-auth-card-title--register">Buat akun siswa</h2>
    <p class="gh-auth-card-subtitle gh-auth-card-subtitle--register">Isi data di bawah untuk mulai belajar.</p>
@endsection

@section('content')
    <form action="/register/student" method="POST" class="gh-form-stack gh-auth-form-register">
        @csrf

        <div class="gh-field-group">
            <label for="name" class="gh-label gh-label-required">Nama lengkap</label>
        ...
        <div class="gh-field-group">
            <label for="email" class="gh-label gh-label-required">Email</label>
        ...
        <button type="submit" class="gh-ref-btn-submit">Daftar & mulai belajar</button>
