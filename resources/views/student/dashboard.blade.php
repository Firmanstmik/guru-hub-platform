...
@section('content')
    <div x-data="{ activeTab: 'semua' }" class="p-4 sm:p-6 max-w-7xl mx-auto space-y-6 sm:space-y-8 animate-fade-in">
        ...
        <div
            class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-5 sm:p-6 rounded-2xl border border-gray-100 shadow-xs">
            <div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div
                class="bg-white p-5 rounded-2xl border border-gray-100 shadow-xs flex items-center space-x-4 sm:col-span-2 lg:col-span-1">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="bg-white p-5 rounded-2xl border border-gray-100 shadow-xs flex items-center justify-between gap-4">
                ...
                <a href="/my-courses"
                    class="px-3 py-2 bg-indigo-50 text-indigo-600 font-bold text-xs rounded-xl hover:bg-indigo-100 transition whitespace-nowrap shrink-0">
                    Buka Materi 📖
                </a>
            </div>
            ...
            <div class="bg-white p-5 rounded-2xl border border-gray-100 shadow-xs flex items-center justify-between gap-4">
                ...
                <a href="{{ url('/quiz/history ') }}"
                    class="px-3 py-2 bg-emerald-50 text-emerald-600 font-bold text-xs rounded-xl hover:bg-emerald-100 transition whitespace-nowrap shrink-0">
                    Lihat Nilai 📊
                </a>
            </div>
        </div>
            ...
            <div class="lg:col-span-2 space-y-4">
                ...
                <div class="space-y-3">
                    @forelse($activeCourses as $course)
                        <div
                            class="bg-white p-5 rounded-2xl border border-gray-100 shadow-xs hover:border-gray-200 transition-all duration-200">
                            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                <div class="flex items-start space-x-4">
                                    {{-- Inisial Singkatan Nama Kelas Dinamis --}}
                                    <div
                                        class="w-10 h-10 rounded-xl bg-indigo-100 text-indigo-600 flex items-center justify-center font-black text-xs shrink-0 uppercase tracking-wider">
                                        {{ $course->short_name }}
                                    </div>
                                    <div>
                                        <span
                                        ...
                                        {{-- MENYESUAIKAN: Mengubah mentor menjadi teacher sesuai dengan nama relasi di model Course --}}
                                        <p class="text-[11px] text-gray-400 font-medium mt-0.5">Mentor:
                                            {{ $course->teacher->name ?? 'Staf Pengajar' }}</p>
                                    </div>
                                </div>
                                <div
                                    class="w-full sm:w-auto sm:text-right flex sm:flex-col justify-between sm:justify-center items-center sm:items-end gap-1.5 border-t sm:border-t-0 pt-3 sm:pt-0 border-gray-50">
                                    <span
                                        class="text-xs font-black text-gray-800 font-mono">{{ $course->student_progress }}%</span>
                            ...
                            <div class="w-full bg-gray-100 h-1.5 rounded-full mt-4 overflow-hidden">
                                <div class="bg-indigo-600 h-1.5 rounded-full transition-all duration-500"
                                    style="width: {{ $course->student_progress }}%"></div>
                            </div>
                        </div>
                    @empty
                    ...
                    <div class="p-4 space-y-4">
                        @foreach ($liveClasses as $live)
                            <div x-show="activeTab === 'semua' || activeTab === 'live'" class="flex items-start space-x-3">
                                <div
                                    class="w-8 h-8 rounded-lg bg-red-50 text-red-500 flex items-center justify-center shrink-0 text-xs font-bold">
                                    🔴
                                </div>
                                <div class="flex-1 border-b border-gray-50 pb-3 last:border-b-0 last:pb-0">
                                    ...
                                    <h4 class="text-xs font-bold text-gray-900 leading-tight mt-0.5">{{ $live->topic }}
                                    </h4>
                                    <p class="text-[10px] text-gray-400 mt-0.5">Link: <a href="{{ $live->link }}"
                                            target="_blank" class="text-indigo-600 hover:underline">Gabung Meet 🌐</a></p>
                                </div>
                            </div>
                        @endforeach
                        ...
                        @foreach ($pendingQuizzes as $quiz)
                            <div x-show="activeTab === 'semua'" class="flex items-start space-x-3">
                                <div
                                    class="w-8 h-8 rounded-lg bg-amber-50 text-amber-500 flex items-center justify-center shrink-0 text-xs font-bold">
                                    📝
                                </div>
                                <div class="flex-1 border-b border-gray-50 pb-3 last:border-b-0 last:pb-0">
                                    <span
                                        class="text-[10px] text-amber-600 font-bold uppercase tracking-wider block">Evaluasi
                                        Belum Selesai</span>
                                    <h4 class="text-xs font-bold text-gray-900 leading-tight mt-0.5">{{ $quiz->title }}
                                    </h4>
                                    <p class="text-[10px] text-gray-400 mt-0.5">Durasi: {{ $quiz->duration_minutes }}
                                        Menit | <a href="{{ url('/materials/' . $quiz->material_id . '/quiz') }}"
                                            class="text-amber-600 font-bold hover:underline">Mulai Tugas ➡️</a></p>
                                </div>
                            </div>
                        @endforeach
