...
@section('content')
    <div class="p-6 max-w-5xl mx-auto space-y-6">
        ...
        @if (session('success'))
            <div class="p-4 bg-emerald-50 border border-emerald-100 rounded-xl text-xs font-semibold text-emerald-800 flex items-center gap-2">
        ...
        @if($bookings->isEmpty())
            <div class="bg-white border border-gray-100 rounded-2xl p-12 text-center space-y-3">
                ...
                <div class="pt-2">
                    <a href="/tampil-kursus" class="inline-flex text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 px-4 py-2 rounded-xl shadow-2xs transition">
                        Cari Program Kelas
                    </a>
                </div>
            </div>
                        ...
                        <div class="space-y-3">
                            <div class="flex flex-wrap items-center gap-2">
                                <span class="font-mono text-xs font-bold text-gray-800 bg-gray-100 px-2.5 py-1 rounded-lg border border-gray-200/60 flex items-center gap-1.5">
                                    ...
                                    <button onclick="copyInvoice('inv-{{ $booking->id }}', this)" class="text-gray-400 hover:text-indigo-600 transition" title="Salin Kode">
                                ...
                                @elseif($booking->status === 'success' || $booking->status === 'active')
                            ...
                            <div>
                                @if($booking->status === 'pending')
                                    <a href="https://wa.me/6287728893916?text=Halo%20Admin,%20saya%20ingin%20konfirmasi%20pembayaran%20kelas%20untuk%20invoice:%20{{ $booking->transaction_code }}" 
                                       target="_blank"
                                       class="inline-flex items-center gap-1.5 px-3 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-xl shadow-2xs transition">
                                ...
                                @elseif($booking->status === 'success' || $booking->status === 'active')
                                    <a href="/my-courses" class="inline-flex items-center gap-1 px-3 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 text-xs font-bold rounded-xl transition">
                                ...
                                @else
                                    <button disabled class="px-3 py-2 bg-gray-50 border border-gray-200 text-gray-400 text-xs font-medium rounded-xl cursor-not-allowed">
                                        Tidak Aktif
                                    </button>
                                @endif
                            </div>
