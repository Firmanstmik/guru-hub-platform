...
@section('content')
    <div class="container mx-auto px-6 py-8">
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 gap-4">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Monitoring dan kelola sinkronisasi link Zoom / Google Meet untuk live
                    class.</p>
            </div>
            ...
            <div class="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
                <form action="/schedules" method="GET" class="flex gap-2 w-full sm:w-auto">
                    <select name="platform" onchange="this.form.submit()"
                        ...
                        <option value="google_meet" {{ request('platform') == 'google_meet' ? 'selected' : '' }}>Google Meet
                        </option>
                    </select>
                    <select name="time_status" onchange="this.form.submit()"
                        class="bg-white border border-gray-300 rounded-lg px-3 py-2 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 focus:outline-none">
                        <option value="">Semua Waktu</option>
                        <option value="upcoming" {{ request('time_status') == 'upcoming' ? 'selected' : '' }}>Akan Datang
                        </option>
                    ...
                    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                        ...
                        <div class="flex-1 min-w-0 space-y-2">
                            <div class="flex items-start gap-3">
                                <div
                                    class="w-10 h-10 rounded-lg flex items-center justify-center text-md flex-shrink-0 border {{ $sched->platform === 'zoom' ? 'bg-blue-50 border-blue-100 text-blue-600' : 'bg-emerald-50 border-emerald-100 text-emerald-600' }}">
                                    📅
                                </div>
                                <div class="min-w-0 space-y-0.5">
                                    <div class="flex flex-wrap items-center gap-2">
                                        <h4 class="text-sm font-bold text-gray-900 truncate max-w-md"
                                            title="{{ $sched->course->title ?? 'Kelas Tidak Ditemukan' }}">
                                            {{ $sched->course->title ?? 'Kelas Tidak Ditemukan' }}
                                        </h4>
                                        @if ($sched->platform === 'zoom')
                                            <span
                                                class="inline-flex px-2 py-0.5 rounded-md text-[10px] font-bold bg-blue-100 text-blue-800">Zoom</span>
                                        @else
                                            <span
                                                class="inline-flex px-2 py-0.5 rounded-md text-[10px] font-bold bg-emerald-100 text-emerald-800">Google
                                                Meet</span>
                                        @endif
                                    </div>
                                    ...
                                    @role('admin')
                                        <div class="text-[11px] text-gray-400">
                                            👨‍🏫 Guru Pengajar: <span
                                                class="font-medium text-gray-600">{{ $sched->course->teacher->name ?? 'Tidak Ada' }}</span>
                                        </div>
                                    @endrole
                                </div>
                            </div>
                        ...
                        <div
                            class="text-xs text-gray-600 leading-relaxed min-w-[200px] border-t lg:border-t-0 pt-2 lg:pt-0 border-gray-50">
                            <span class="block text-[10px] text-gray-400 uppercase tracking-wider font-bold mb-0.5">Waktu
                                Pelaksanaan</span>
                            <span
                                class="font-semibold text-gray-900">{{ $sched->start_time->isoFormat('dddd, D MMMM YYYY') }}</span>
                            <span class="block text-gray-500 text-[11px]">{{ $sched->start_time->format('H:i') }} -
                                {{ $sched->end_time->format('H:i') }} WIB</span>
                        </div>
                        ...
                        <div
                            ...
                            <div>
                                ...
                                @else
                                    <span
                                        class="text-xs text-gray-400 italic bg-gray-50 px-3 py-2 rounded-xl border border-gray-100 block">Belum
                                        diset</span>
                                @endif
                            </div>
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                {{-- <form action="/schedules/{{ $sched->id }}" method="POST"
                                    onsubmit="return confirm('Hapus jadwal sesi live ini?')" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
            ...
            @empty
                <div class="bg-white border border-gray-100 rounded-xl p-12 text-center space-y-3 shadow-xs">
                    ...
                    <div class="space-y-1">
                        ...
                        <p class="text-xs text-gray-400 max-w-xs mx-auto">Tidak ditemukan jadwal kelas aktif atau pertemuan
                            sinkronus yang terdaftar dalam database saat ini.</p>
                    </div>
                </div>
            @endforelse
        </div>
                ...
                <form action="/schedules" method="POST">
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                <label class="block text-xs font-semibold text-gray-700 mb-1">Topik Sesi
                                    Pembelajaran</label>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label class="block text-xs font-semibold text-gray-700 mb-1">Waktu Selesai
                                        Sesi</label>
                            ...
                            <div>
                                <label class="block text-xs font-semibold text-gray-700 mb-1">Platform Video
                                    Conference</label>
                            ...
                            <div>
                                <label class="block text-xs font-semibold text-gray-700 mb-1">Tautan Room Meeting (Link
                                    URL)</label>
                    ...
                    <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse gap-2">
                        <button type="submit"
                            ...
                            Jadwal</button>
                        <button type="button" onclick="toggleModal('addScheduleModal')"
                ...
                <form id="editForm" method="POST">
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                <label class="block text-xs font-semibold text-gray-700 mb-1">Tautan Room Meeting (Link
                                    URL)</label>
                                <input type="url" id="edit_meeting_link" name="meeting_link"
                                    class="w-full border-gray-300 rounded-lg text-sm p-2.5 border">
                            </div>
                        </div>
                    </div>
                    <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse gap-2">
                        <button type="submit"
                            ...
                            Jadwal</button>
                        <button type="button" onclick="toggleModal('editScheduleModal')"
                            class="mt-3 sm:mt-0 w-full sm:w-auto bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 font-medium px-4 py-2 rounded-lg text-sm transition">Batal</button>
                    </div>
                </form>
            </div>
    ...
    <script>
        function handleOpenEditModal(button) {
            ...
            openEditModal(scheduleData); // Teruskan ke fungsi pembuka modal bawaan Anda
        }
