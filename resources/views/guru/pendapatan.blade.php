                ...
                <table class="min-w-full divide-y divide-gray-200 text-left">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-xs font-semibold text-gray-500 uppercase">Hak Pendapatan ({{ $teacherPercentage * 100 }}%)</th>
                        ...
                        @forelse($earnings as $payment)
                            <tr class="hover:bg-gray-50 transition duration-150">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap font-bold text-gray-900">
                                    {{-- Kalkulasi porsi pendapatan guru secara langsung --}}
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-xs font-medium flex justify-end">
                                    @if ($payment->earning_status === 'pending')
                                        ...
                                        @else
                                            <span class="text-xs text-amber-600 italic font-medium">Menunggu Proses Transfer Admin</span>
                                        @endrole
                                    @else
                                        <span class="text-xs text-gray-400 italic">Selesai diproses ({{ $payment->updated_at->format('d/m/Y') }})</span>
                                    @endif
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="{{ auth()->user()->hasRole('admin') ? '6' : '5' }}"
                                    class="px-6 py-12 text-center text-gray-500">Belum ada rekaman bagi hasil komisi guru yang tercatat.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
