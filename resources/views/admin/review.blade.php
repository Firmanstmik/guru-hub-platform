    ...
    <div class="container mx-auto px-6 py-8">
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 gap-4">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Pantau umpan balik dari murid dan lakukan moderasi terhadap ulasan yang
            ...
            <div class="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
                ...
                <form action="/reviews" method="GET" class="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                    <input type="text" name="search" value="{{ request('search') }}"
                        placeholder="Cari isi ulasan / murid..."
        ...
        <div class="w-full overflow-x-auto rounded-2xl border border-slate-200 shadow-xs bg-white">
            <table class="min-w-full divide-y divide-slate-200 text-sm whitespace-nowrap">
                ...
                <tbody class="bg-white divide-y divide-gray-100 text-sm">
                        ...
                        <tr class="hover:bg-gray-50 transition duration-150">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right font-medium flex justify-end mt-1">
                                ...
                                <form action="/reviews/{{ $review->id }}" method="POST"
                                    onsubmit="return confirm('Apakah Anda yakin ingin menghapus ulasan ini secara permanen dari sistem? Tindakan ini tidak dapat dibatalkan.')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                                        class="bg-rose-50 hover:bg-rose-100 text-rose-600 px-3 py-1.5 rounded-md text-xs font-semibold border border-rose-200 transition duration-150">
                                        Hapus / Take Down
                                    </button>
                                </form>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-gray-500">Belum ada data ulasan atau
                                rating dari murid yang masuk.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
