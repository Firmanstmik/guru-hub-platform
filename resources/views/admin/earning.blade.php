            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 text-left">
                    <thead class="bg-gray-50">
                        <tr>
                            {{-- Sembunyikan kolom Nama Guru jika yang login adalah Guru --}}
                            @role('admin')
                            ...
                            <th class="px-6 py-3 text-xs font-semibold text-gray-500 uppercase">Hak Pendapatan (Bagi Hasil)
                            </th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-100 text-sm">
                        @forelse($earnings as $earn)
                            <tr class="hover:bg-gray-50 transition duration-150">
                                {{-- Tampilkan data nama Guru hanya untuk sisi Admin --}}
                                @role('admin')
                                    <td class="px-6 py-4 whitespace-nowrap font-medium text-gray-900">
                                        {{ $earn->teacher->name ?? 'Tidak Ada' }}
                                    </td>
                                @endrole
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-xs font-medium flex justify-end">
                                    @if ($earn->status === 'pending')
                                        {{-- PROTEKSI UTAMA: Hanya Admin yang boleh melihat dan menekan tombol transfer ini --}}
                                        @role('admin')
                                            <form action="/earnings/{{ $earn->id }}/status" method="POST"
                                                ...
                                                <button type="submit"
                                                    class="bg-indigo-600 hover:bg-indigo-700 text-white font-medium px-3 py-1.5 rounded-md shadow-xs transition">
                                                    Tandai Sudah Ditransfer
                                                </button>
                                            </form>
                                        @else
                                            {{-- Tampilan di sisi Guru jika statusnya masih pending --}}
                                            <span class="text-xs text-amber-600 italic font-medium">Menunggu Proses Transfer
                                                Admin</span>
                                        @endrole
                                    @else
                                        <span class="text-xs text-gray-400 italic">Selesai diproses
                                            ({{ $earn->updated_at->format('d/m/Y') }})</span>
                                    @endif
                                </td>
                            </tr>
                        @empty
