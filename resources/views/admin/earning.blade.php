...
@section('content')
    <div class="container mx-auto px-6 py-8">
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 gap-4">
            <div>
                <h3 class="text-gray-700 text-3xl font-semibold">
                    @role('admin') Manajemen Pendapatan & Bagi Hasil @else Pendapatan & Bagi Hasil Anda @endrole
                </h3>
                <p class="text-sm text-gray-500 mt-1">
                    @role('admin')
                        Pantau porsi komisi pengajar, lacak keuntungan platform, dan kelola konfirmasi transfer pencairan dana guru.
                    @else
        ...
        @if (session('success'))
            <div class="mb-6 p-4 bg-emerald-50 border border-emerald-100 rounded-xl text-sm font-semibold text-emerald-800 flex items-center gap-2">
        ...
        <div class="grid grid-cols-1 {{ auth()->user()->hasRole('admin') ? 'md:grid-cols-3' : 'sm:grid-cols-2' }} gap-5 mb-8">
            <!-- Card Pending -->
            <div class="bg-white p-6 rounded-xl shadow-xs border border-gray-100 flex items-center justify-between">
                <div>
                    <p class="text-xs font-semibold text-gray-400 uppercase tracking-wider">
                        @role('admin') Total Pending Guru @else Pending (Belum Dicairkan) @endrole
                    </p>
            ...
            <div class="bg-white p-6 rounded-xl shadow-xs border border-gray-100 flex items-center justify-between">
                <div>
                    <p class="text-xs font-semibold text-gray-400 uppercase tracking-wider">
                        @role('admin') Total Paid Guru @else Paid (Sudah Ditransfer) @endrole
                    </p>
            ...
            @role('admin')
                ...
                <div class="bg-white p-6 rounded-xl shadow-xs border border-gray-100 flex items-center justify-between">
                    <div>
                        <p class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Pendapatan Bersih Sistem ({{ $platformPercentage * 100 }}%)</p>
                        <h4 class="text-2xl font-bold text-indigo-600 mt-1">Rp {{ number_format($platformEarnings, 0, ',', '.') }}</h4>
                    </div>
                    ...
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-xs font-semibold text-gray-500 uppercase">
                                @role('admin') Rincian Pembagian Hasil @else Hak Pendapatan Anda ({{ $teacherPercentage * 100 }}%) @endrole
                            </th>
                                    ...
                                    @role('admin')
                                        <div class="space-y-1">
                                            <div class="flex items-center gap-1.5">
                                                <span class="px-1.5 py-0.2 bg-gray-100 text-gray-600 rounded text-[10px] font-semibold font-mono">Guru ({{ $teacherPercentage * 100 }}%)</span>
                                                <span class="font-bold text-gray-950">Rp {{ number_format($payment->amount * $teacherPercentage, 0, ',', '.') }}</span>
                                            </div>
                                            <div class="flex items-center gap-1.5">
                                                <span class="px-1.5 py-0.2 bg-indigo-50 text-indigo-600 rounded text-[10px] font-semibold font-mono">Sistem ({{ (1 - $teacherPercentage) * 100 }}%)</span>
                                                <span class="font-medium text-indigo-600">Rp {{ number_format($payment->amount * (1 - $teacherPercentage), 0, ',', '.') }}</span>
                                            </div>
                                        </div>
                                    @else
