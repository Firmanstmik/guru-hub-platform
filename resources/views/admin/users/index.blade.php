    ...
    <div class="container mx-auto px-6 py-8">
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 gap-4">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Kelola hak akses pengguna, monitor status keaktifan, dan lakukan
                    tindakan pembekuan akun pelanggar.</p>
            </div>
        ...
        <div class="w-full overflow-x-auto rounded-2xl border border-slate-200 shadow-xs bg-white">
            <table class="min-w-full divide-y divide-slate-200 text-sm whitespace-nowrap">
                ...
                <tbody class="bg-white divide-y divide-gray-100 text-sm">
                        ...
                        <tr class="hover:bg-gray-50 transition duration-150">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <form action="{{ url('/users/' . $user->id . '/update-role') }}" method="POST"
                                    ...
                                    @csrf
                                    <select name="role" onchange="this.form.submit()"
                                        class="text-xs bg-gray-50 border border-gray-200 rounded px-2 py-1 font-medium text-gray-700 focus:border-indigo-500 focus:bg-white"
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-xs text-gray-600">
                                @if ($user->hasRole('teacher'))
                                    <span class="font-semibold text-indigo-600">{{ $user->teacherCourses->count() }}</span>
                                    Kelas Dibuat
                                @else
                                    <span
                                        class="font-semibold text-emerald-600">{{ $user->enrolledCourses->count() }}</span>
                                    Kelas Diikuti
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if ($user->is_active)
                                    <span
                                        class="px-2.5 py-0.5 text-xs font-semibold bg-emerald-100 text-emerald-800 rounded-full">Aktif</span>
                                @else
                                    <span
                                        class="px-2.5 py-0.5 text-xs font-semibold bg-rose-100 text-rose-800 rounded-full">Suspended</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-xs font-medium">
                                <div class="flex justify-end items-center gap-3">
                                    <a href="{{ url('/users/' . $user->id . '/edit') }}"
                                        class="text-amber-600 hover:text-amber-900 font-semibold">Edit</a>
                                    ...
                                    @if (auth()->id() !== $user->id)
                                        <form action="{{ url('/users/toggle', $user->id) }}" method="POST" class="inline">
                                            ...
                                            @if ($user->is_active)
                                                <button type="submit"
                                                    class="text-rose-600 hover:text-rose-900 font-semibold">Suspend</button>
                                            @else
                                                <button type="submit"
                                                    class="text-emerald-600 hover:text-emerald-900 font-semibold">Aktifkan</button>
                                            @endif
                                        </form>
                                        ...
                                        <form action="{{ url('/users/' . $user->id) }}" method="POST" class="inline"
                                            ...
                                            @method('DELETE')
                                            <button type="submit"
                                                class="text-gray-400 hover:text-rose-600 font-semibold">Hapus</button>
                                        </form>
                                    @else
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-gray-500">Data pengguna tidak
                                ditemukan atau filter tidak cocok.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
