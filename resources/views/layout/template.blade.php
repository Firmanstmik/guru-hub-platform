...
<body class="bg-gray-50 antialiased font-sans md:overflow-hidden">
    ...
    <div class="min-h-screen flex flex-col md:flex-row md:h-screen md:overflow-hidden"
        ...
        x-data="{
            ...
            init() {
                this.$watch('collapsed', value => {
                    localStorage.setItem('adminSidebarCollapsed', value ? 'true' : 'false');
                });
            }
        }"
        @endif>
        ...
        <main class="flex-1 w-full min-w-0 min-h-0 overflow-x-hidden md:overflow-y-auto transition-all duration-300"
            @if (auth()->user()->hasRole('admin'))
            :class="collapsed ? 'md:pl-[4.75rem]' : 'md:pl-64'"
            @endif>
