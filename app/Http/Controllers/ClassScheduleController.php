...
class ClassScheduleController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            if ($user->hasRole('admin')) {
                ...
                $materials = CourseMaterial::whereHas('course', function ($q) use ($user) {
                    $q->where('teacher_id', $user->id);
                })->orderBy('title')->get();
            }

            // Return view dengan menyertakan variabel $materials
            if ($user->hasRole('admin')) {
                return view('admin.schedules', compact('schedules', 'courses', 'materials'));
            } elseif ($user->hasRole('guru')) {
                return view('guru.schedules', compact('schedules', 'courses', 'materials'));
            } else {
                abort(403, 'Anda tidak memiliki hak akses untuk halaman ini.');
            }
        } catch (Exception $e) {
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            ...
            'material_id.exists'        => 'Materi yang dipilih tidak valid atau tidak ditemukan.',
